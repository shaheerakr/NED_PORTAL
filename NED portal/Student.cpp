...
void Student::viewMarks(string userName)
{
hello:
	...
	fin1.open("Members Details\\" + userName + " course.txt");
	if(choice=='s')
		fin2.open("Members Details\\" + userName + " seasional marks.txt");
	if(choice=='f')
		fin2.open("Members Details\\" + userName + " final marks.txt");
	ui.clearArea(45, 13, 115, 14);
	while (!fin1.eof() || !fin2.eof())
	{
		getline(fin1, course);
		...
		else
		{
			i++;
			ui.gotoxy(45, i + 12);
			cout << "marks for " << course << " is " << attendence;
		}
	}
}

void Student::viewCourseOutline(string userName)
{
	ifstream fin1, fin2, fin3;
	int j,tag1;
	string c3, c4, c5,depart,year;
	fin1.open("Usernames Student.txt");
	fin2.open("Department Student.txt");
	fin3.open("Year Student.txt");
	ui.gotoxy(45, 15);
	...
	ui.clearArea(45,13,115,11);
	ui.gotoxy(45,15); 
	cout << "Your Course Outline is being generated";
	ui.delay(500);
	system(("notepad \"" "courseoutline"+depart+year+"" "\"" ".txt").c_str());
}