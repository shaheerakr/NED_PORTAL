...
hello:
	ui.clearArea(45, 13, 115, 11);
	int i = 0, tag = 0, tag1 = 0, j=0, sel, k=13,attendence;
...
void Teacher::setMarks(string year, string userName)
{
hello:
	ui.clearArea(35, 13, 119, 15);
	char choice = 'z';
	...
	fin.open("Members Details\\" + st[sel - 1] + " course.txt");
	if(choice=='s')
		fout.open("Members Details\\" + st[sel - 1] + " Seasional marks.txt", std::fstream::in | std::fstream::out | std::fstream::app);
	if (choice == 'f')
